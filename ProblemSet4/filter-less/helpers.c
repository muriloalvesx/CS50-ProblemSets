...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        ...
        {
            RGBTRIPLE *pixel = &image[i][j];
            int sepiaRed = round(0.189 * (*pixel).rgbtBlue + 0.769 * (*pixel).rgbtGreen +
                                 0.393 * (*pixel).rgbtRed);
            int sepiaGreen = round(0.168 * (*pixel).rgbtBlue + 0.686 * (*pixel).rgbtGreen +
                                   0.349 * (*pixel).rgbtRed);
            int sepiaBlue = round(0.131 * (*pixel).rgbtBlue + 0.534 * (*pixel).rgbtGreen +
                                  0.272 * (*pixel).rgbtRed);
